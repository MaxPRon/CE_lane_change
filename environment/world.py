...
class World:
    ...
    def render(self):
        ...
        ax2.set_ylim([0, (lane - 1) * self.road_width + self.road_width * 0.5 + 2])
        #print(ego.x)
        ax2.set_xlim([ego.x-100,ego.x+100])
    ...
    def step(self,action):
        ...
        return self.vehicle_list, self.reward, self.done
    ...
    def dist_control(self,id):
        ...
        if len(vehicle_list_sec) == 0:
            ...
            #if id == 0:
            #    print("Dist: ", dist, "Ego-Pos: ",self.vehicle_list[id].x, "X-Front: ",x_front, "Safety distance",self.s0)
    ...
    def reward_function(self):
        ...
        self.reward -= (self.speed_limit - self.vehicle_list[0].v)*3
        if self.vehicle_list[0].y in {2,6,10,14,18,22}:
            self.reward += 1
            #print("Y-position:",self.vehicle_list[0].y," Getting reward")
        else:
            self.reward -= 1

        if self.vehicle_list[0].y == 2:
            self.reward += 2
            #print("Y-position:", self.vehicle_list[0].y, " Getting reward extra")

        if self.vehicle_list[0].y > ((self.n_lanes - 1) * self.road_width + self.road_width * 0.5)+0.25 or self.vehicle_list[0].y < (self.road_width*0.45):
            self.reward -= 100
            self.done = True
            #print("Fucked up")
